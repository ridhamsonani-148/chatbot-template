...
export class CatholicCharitiesStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: CatholicCharitiesStackProps) {
    ...
    try {
      ...
      throw new Error(
        `Failed to retrieve IAM Identity Center ARN: ${error.message}. Ensure IAM Identity Center is enabled and the CDK execution role has 'sso:ListInstances' permission.`
      );
    }
    ...
    const qBusinessIndex = new qbusiness.CfnIndex(this, "QBusinessIndex", {
      ...
      type: "STARTER", // Changed to STARTER
